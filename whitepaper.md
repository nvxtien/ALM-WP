...
## 2. High level Blockchain design ##
...
This section describes **MainChain**, ALM's Hyperledger Fabric (HPL) based blockchain system.
...
## Preliminaries
During ICO on Ethereum network, the Host issues # Introduction
## 1. Loyalty Management ##
...
\mathcal{T}`$ amount of **FToken**. After ICO, the Host will not be able to generate any new FToken without agreement from the general consensus on MainChain.

We then denote # Introduction
## 1. Loyalty Management ##
...
\mathcal{T}_i`$ as the amount of FToken held by an investor # Introduction
## 1. Loyalty Management ##
...
1. The host can *mint* an unlimited number of **FCoin**. This is a **fresh** FCoin
2. A Merchant M1 buys fresh FCoin from the host. The price of FCoin is **fixed** according to the general consensus of the MainChain. For example: 1 FCoin = 1 USD
3. An *Exchange* transaction:  
    3.1. A customer issues an *Exchange* transaction to exchange Loyalty Point (LP1) from M1 to a private ChainCoin.  
    3.2. M1 must pay a **fixed** amount of **fresh FCoin** from the MainChain for the host in order to get the exchange transaction commited. This is performed via our Bridge Protocol.    
    3.3. The PrivateChain issues a proof that M1 has paid the required transaction fee in FCoin and submit this proof to MainChain via the Bridge Protocol. This coin is marked as **used FCoin**.  
    3.4. Customer balance on PrivateChain is updated with new ChainCoin.  
4. *ILP Transaction*:  
    4.1. Depending on # Introduction
## 1. Loyalty Management ##
...
\mathcal{T}_i`$, the MainChain divides the used FCoin received from the Bridge Protocol and sends to investors using **Ripple Interledger Protocol (ILP)**. In particular, if we denote # Introduction
## 1. Loyalty Management ##
...
\mathcal{P}_i`$ as the amount of used FCoin that an investor # Introduction
## 1. Loyalty Management ##
...
i`$ will receive and # Introduction
## 1. Loyalty Management ##
...
u`$ as the number of used FCoin from step 3.3. We will have the following formular.

    ```math
    \mathcal{P}_i = u * \frac{\mathcal{T}_i}{\mathcal{T}}
    ```

    4.2. The used FCoin after being sent via ILP must be burnt out of the system.    
